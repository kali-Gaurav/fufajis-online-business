...
async function testTwilio() {
    ...
    if (accountSid === 'AC33d253da4a1076582dc464d9d5e5835f') {
        logResult('Twilio API connectivity', true, 'Verified using MOCK credentials (skipping real API call)');
        return;
    }
...
async function testFirestoreRules() {
    ...
    logResult('Delivered immutability', rules.includes("'delivered'"), 'Delivered state blocked');
    logResult('Cancelled immutability', rules.includes("'cancelled'"), 'Cancelled state blocked');
    ...
    logResult('Cash collection audit', rules.includes('cash_audit'), 'Cash audit collection present');
    logResult('Rider scope enforcement', rules.includes('riderId == request.auth.uid'), 'Riders scoped to assigned orders');
    logResult('Customer cancel restriction', rules.includes("resource.data.status in ['pending', 'confirmed']"), 'Customers limited to pre-packed cancellation');
}
