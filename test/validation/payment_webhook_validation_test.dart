...
class WebhookProcessor {
  ...
  bool validateSignature(
    ...
    final expectedSignature = Hmac(sha256, utf8.encode(secret)).convert(utf8.encode(payload));
    if (expectedSignature.toString() == signature) return true;
    if (flatMap != null) {
      final expectedSignatureFlat = Hmac(
        sha256,
        utf8.encode(secret),
      ).convert(utf8.encode(jsonEncode(flatMap)));
  ...
  Future<({bool success, String reason, String? orderId})> processWebhook(
    ...
    switch (event.eventType) {
      ...
      case 'payment.failed':
        ...
        break;
      default:
        return (success: false, reason: 'Unknown event type: ${event.eventType}', orderId: null);
    }
  ...
  List<MockLedgerEntry> getLedgerForOrder(String orderId) {
    return ledger.values.where((entry) => entry.orderId == orderId).toList();
  }
}
...
void main() {
  group('Payment Webhook Validation Tests', () {
    ...
    test('Scenario 1: Network timeout on webhook delivery triggers retry queue', () async {
      ...
      final order = MockOrder(orderId: orderId, customerId: 'CUST-001', amount: 5000);
      ...
      final webhookEvent = MockWebhookEvent(
        ...
        payload: {
          'payment': {'id': 'PAY-001', 'amount': 500000, 'status': 'authorized'},
          'order': {'id': orderId},
        },
      );
    ...
    test('Scenario 2: Duplicate webhook is idempotent - no double-charging', () async {
      ...
      final orderId = 'ORD-${DateTime.now().millisecondsSinceEpoch}';
      final order = MockOrder(orderId: orderId, customerId: 'CUST-002', amount: 10000);
      processor.orders[orderId] = order;
      ...
      final webhookEvent = MockWebhookEvent(
        ...
        payload: {
          'payment': {'id': 'PAY-DUP-001', 'amount': 1000000, 'status': 'authorized'},
          'order': {'id': orderId},
        },
      );
      ...
      expect(ledgerEntries2.every((e) => e.amount == 10000), true);
    });
    ...
    test('Scenario 3: Old failure webhook does not overwrite new success', () async {
      ...
      final orderId = 'ORD-${DateTime.now().millisecondsSinceEpoch}';
      final order = MockOrder(orderId: orderId, customerId: 'CUST-003', amount: 7500);
      processor.orders[orderId] = order;
      ...
      final successWebhook = MockWebhookEvent(
        ...
        signature: _generateSignature({
          'payment_id': 'PAY-SUCCESS-001',
          'order_id': orderId,
        }, webhookSecret),
        payload: {
          'payment': {'id': 'PAY-SUCCESS-001', 'amount': 750000, 'status': 'authorized'},
          'order': {'id': orderId},
        },
      );
    ...
    test('Scenario 4: Firestore failure rolls back - no partial updates', () async {
      ...
      final orderId = 'ORD-${DateTime.now().millisecondsSinceEpoch}';
      final order = MockOrder(orderId: orderId, customerId: 'CUST-004', amount: 3000);
      processor.orders[orderId] = order;
      ...
      final webhookEvent = MockWebhookEvent(
        ...
        createdAt: DateTime.now(),
        signature: _generateSignature({
          'payment_id': 'PAY-WF-001',
          'order_id': orderId,
        }, webhookSecret),
        payload: {
          'payment': {'id': 'PAY-WF-001', 'amount': 300000, 'status': 'authorized'},
          'order': {'id': orderId},
        },
      );
      ...
      expect(failingProcessor.retryQueue.contains(webhookEvent.eventId), true); // Added to retry
    });

    // ========================================================================
    // SCENARIO 5: 100 Concurrent Payment Webhooks (Race Conditions)
    // ========================================================================
    test('Scenario 5: 100 concurrent webhooks - no race conditions, no data loss', () async {
      // Arrange: Create 100 orders
      final orders = <String, MockOrder>{};
      for (int i = 0; i < 100; i++) {
        final orderId = 'ORD-CONCURRENT-$i';
        final order = MockOrder(orderId: orderId, customerId: 'CUST-$i', amount: 5000 + (i * 100));
      ...
      final futures = webhooks.map((webhook) async {
        await Future.delayed(Duration(milliseconds: Random().nextInt(10))); // Random delay
      ...
      final totalInLedger = processor.ledger.values.fold(0.0, (sum, entry) => sum + entry.amount);
      final expectedTotal = Iterable<int>.generate(
        100,
    ...
    // ========================================================================
    test('Idempotency: Same payment_id never creates multiple ledger entries', () async {
      const orderId = 'ORD-IDEMPOTENCY-001';
      const paymentId = 'PAY-IDEMPOTENCY-001';
      const eventId = 'EV-IDEMPOTENCY-001';

      final order = MockOrder(orderId: orderId, customerId: 'CUST-IDEM', amount: 2500);
      processor.orders[orderId] = order;
      ...
      final webhook = MockWebhookEvent(
        ...
        payload: {
          'payment': {'id': paymentId, 'amount': 250000, 'status': 'authorized'},
          'order': {'id': orderId},
        },
      );
    ...
    test('Webhook signature validation prevents forged payments', () async {
      ...
      final order = MockOrder(orderId: orderId, customerId: 'CUST-SIG', amount: 1000);
      processor.orders[orderId] = order;
      ...
      final webhookEvent = MockWebhookEvent(
        ...
        amount: 1000,
        createdAt: DateTime.now(),
        signature: 'INVALID-FORGED-SIGNATURE-XXXXXXXXXXXXXXXXXXXX', // Forged
        payload: {
          'payment': {'id': 'PAY-SIG-001', 'amount': 100000, 'status': 'authorized'},
          'order': {'id': orderId},
        },
      );
