...
void main() {
  group('CircuitBreaker Tests', () {
    test('Successful tasks keep circuit closed', () async {
      final breaker = CircuitBreaker('TestService', config: const CircuitBreakerConfig(failureThreshold: 3));
    ...
    test('Fallback is executed when circuit is open', () async {
      final breaker = CircuitBreaker('TestService', config: const CircuitBreakerConfig(failureThreshold: 1));
    ...
    test('Circuit transitions to half-open after timeout', () async {
      ...
      final breaker = CircuitBreaker('TestService', config: const CircuitBreakerConfig(failureThreshold: 1, resetTimeout: Duration(milliseconds: 100)));
