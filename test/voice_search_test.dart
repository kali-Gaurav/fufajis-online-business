...
void main() {
  ...
  group('Dual-Language Search Matching Tests', () {
    ...
    test(
      'Hindi query searches for आलू (Aloo) should return Fresh Potatoes',
      () {
    ...
    test('English query searches for milk should return Fresh Toned Milk', () {
      ...
      expect(results, isNotEmpty);
      expect(results.any((product) => product.name.contains('Milk')), isTrue);
    });

    test(
      'Hindi query searches for दूध (Doodh) should return Fresh Toned Milk',
      () {
    ...
    test(
      'Query with trailing punctuation should strip it and find the product',
      () {
        // Common voice transcription artifact: adding a period or Hindi full stop (।)
        final englishPunctuationResults = productProvider.searchProducts(
          'milk.',
        );
        ...
        expect(
          englishPunctuationResults.any((p) => p.name.contains('Milk')),
          isTrue,
        );

        expect(hindiPunctuationResults, isNotEmpty);
        expect(
          hindiPunctuationResults.any((p) => p.name.contains('Milk')),
          isTrue,
        );
      },
    );
    ...
    test(
      'Hindi script voice query with conjunction should match multiple products',
      () {
