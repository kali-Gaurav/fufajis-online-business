...
class DailyBriefingUI extends StatelessWidget {
          ...
          children: [
            ...
            if (briefing.metrics.isNotEmpty) ...[
              Wrap(
                spacing: 16,
                runSpacing: 16,
                children: briefing.metrics.entries.map((e) => _buildMetric(e.key, e.value.toString())).toList(),
              ),
              const SizedBox(height: 20),
            ],
            ...
            if (briefing.urgentActionItems.isNotEmpty) ...[
              const Text('URGENT ACTIONS', style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
              const SizedBox(height: 8),
              ...briefing.urgentActionItems.map((item) => Padding(
            ...
            if (briefing.insights.isNotEmpty) ...[
              const Text('AI INSIGHTS', style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
              const SizedBox(height: 8),
              ...briefing.insights.map((item) => Padding(
                padding: const EdgeInsets.only(bottom: 8.0),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Expanded(child: Text(item, style: const TextStyle(color: Colors.white, fontStyle: FontStyle.italic))),
                  ],
                ),
              )),
              const SizedBox(height: 16),
  ...
  Widget _buildMetric(String label, String value) {
    ...
    final formattedLabel = label.replaceAllMapped(RegExp(r'[A-Z]'), (match) => ' ${match.group(0)}').trim();
    final capitalizedLabel = formattedLabel[0].toUpperCase() + formattedLabel.substring(1);
    
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(capitalizedLabel, style: TextStyle(color: Colors.white.withValues(alpha: 0.7), fontSize: 12)),
