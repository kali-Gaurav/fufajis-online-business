...
class CustomerRetentionService {
  ...
  Future<void> sendReactivationIncentive({
    ...
    try {
      ...
      await _db.collection('notification_queue').add({
        ...
        'userId': userId,
        'payload': {'title': 'We Miss You! ₹${amount.toInt()} Free Balance 🎁', 'body': message},
        'status': 'pending',
  ...
  Future<Map<String, dynamic>> calculateRecoveryStats() async {
    try {
      ...
      final recentIncentivesSnap = await _db
          .collection('users')
          .where(
            'lastIncentiveSentAt',
      ...
      for (final doc in recentIncentivesSnap.docs) {
        ...
        if (lastOrderAt != null &&
            ...
            lastOrderAt.isAfter(lastIncentiveAt)) {
          ...
          if (orderSnap.docs.isNotEmpty) {
            recoveredRevenue += ((orderSnap.docs.first.data()['totalAmount'] as num?) ?? 0.0)
                .toDouble();
          }
        }
      }
      ...
      return {
        ...
        'recoveryRate': totalIncentivized > 0
            ? (successfullyRecovered / totalIncentivized) * 100
            : 0.0,
