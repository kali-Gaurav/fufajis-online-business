...
class _BahiKhataScreenState extends State<BahiKhataScreen> {
  ...
  Widget _buildSummaryHeader() {
    return Container(
      ...
      child: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('users').where('creditBalance', isGreaterThan: 0).snapshots(),
        builder: (context, snapshot) {
          double total = 0;
          int count = 0;
          if (snapshot.hasData) {
          ...
          return Column(
            children: [
              ...
              Text('from $count customers', style: const TextStyle(color: Colors.white60, fontSize: 12)),
            ],
          );
        }
      ),
    ...
    return Card(
      ...
      child: ListTile(
        ...
        leading: CircleAvatar(
          ...
          child: Text(user.name?[0].toUpperCase() ?? '?', style: const TextStyle(color: AppTheme.secondary, fontWeight: FontWeight.bold)),
        ),
        ...
        trailing: Column(
          ...
          children: [
            ...
            Text('₹${user.creditBalance}', style: const TextStyle(color: AppTheme.error, fontWeight: FontWeight.bold, fontSize: 18)),
          ],
        ),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            Text(user.name ?? 'Customer', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Text('Outstanding: ₹${user.creditBalance}', style: const TextStyle(color: AppTheme.error, fontSize: 24, fontWeight: FontWeight.w900)),
            ...
            Expanded(
              child: StreamBuilder<List<KhataTransaction>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (txs.isEmpty) return const Center(child: Text('No transaction history.'));
  ...
  void _showAddManualCredit() {
    // Dialog to pick a customer and add manual credit (e.g. offline purchase)
  }
  ...
  void _showKhataInfo() {
    showAboutDialog(
      ...
      children: [
        const Text('This digital ledger replaces physical notebooks. '
