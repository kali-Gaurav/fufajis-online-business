...
class _SyncHealthDashboardState extends State<SyncHealthDashboard> {
  ...
  Future<void> _fetchMetrics() async {
    ...
    try {
      // 1. Conflicts
      final conflictsSnap = await _db.collectionGroup('inventory_alerts')
      ...
      final runSnap = await _db.collection('reconciliation_runs')
      ...
      if (runSnap.docs.isNotEmpty) {
        ...
        if (completed != null) {
          ...
          _lastReconTime = "${dt.day}/${dt.month} ${dt.hour.toString().padLeft(2, '0')}:${dt.minute.toString().padLeft(2, '0')}";
        }
        _lastReconStatus = (data['status'] as String? ?? 'unknown').toUpperCase();
      }

      // 5. Unresolved anomalies count
      final anomaliesSnap = await _db.collection('transaction_integrity_events')
  ...
  Future<void> _triggerManualReconciliation() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error triggering reconciliation: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Sync & Reconciliation NOC', style: TextStyle(fontWeight: FontWeight.w700)),
        actions: [
          IconButton(
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: _lastReconStatus == 'CLEAN' 
                                    ...
                                    : (_lastReconStatus == 'ANOMALIES_DETECTED' ? Colors.orange.withAlpha((255 * 0.2).toInt()) : Colors.grey.withAlpha((255 * 0.2).toInt())),
                              ...
                              child: Text(
                                ...
                                style: TextStyle(
                                  ...
                                  color: _lastReconStatus == 'CLEAN' 
                                      ? Colors.green 
                                      : (_lastReconStatus == 'ANOMALIES_DETECTED' ? Colors.orange : Colors.grey),
                                ),
                              ),
                            ),
                          ],
                        ...
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            const Text('Last Completed Time:', style: TextStyle(color: Colors.grey)),
                            Text(_lastReconTime, style: const TextStyle(fontWeight: FontWeight.w500)),
                          ],
                        ),
                        ...
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            const Text('Unresolved Ledger Anomalies:', style: TextStyle(color: Colors.grey)),
                            Text(
                              _unresolvedAnomalies.toString(), 
                              style: TextStyle(
                                fontWeight: FontWeight.bold,
                                color: _unresolvedAnomalies > 0 ? AppTheme.error : AppTheme.success,
                              ),
                            ),
                          ],
                        ),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            label: Text(
                              _isReconciling ? 'Running Subsystem Audits...' : 'Execute Reconciliation Run',
                              style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold),
                            ),
                            onPressed: (_isReconciling || _isLoading) ? null : _triggerManualReconciliation,
                          ),
                        ),
                      ],
                    ),
