...
class _BranchManagerCommandCenterState extends State<BranchManagerCommandCenter> {
            ...
            child: Column(
              ...
              children: [
                ...
                QuickActionFramework(
                  actions: [
                    QuickAction(icon: Icons.shopping_cart, label: 'Create Purchase Request', onTap: () {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Create Purchase Request')));
                    }),
                    QuickAction(icon: Icons.person, label: 'Assign Employee', onTap: () {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Assign Employee')));
                    }),
                    QuickAction(icon: Icons.fact_check, label: 'Approve PO', onTap: () {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Approve PO')));
                ...
                const Text('BRANCH HEALTH SNAPSHOT', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: AppTheme.grey600)),
                const SizedBox(height: 12),
                if (health != null) _buildBranchHealthSnapshot(health),
                const SizedBox(height: 32),

                // Section 4: Inventory Action Center
                const Text('INVENTORY ACTION CENTER', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: AppTheme.grey600)),
                const SizedBox(height: 12),
                _buildInventoryActionCard('Rice 5kg', '7 Units', '2 Days', OperationalStatus.warning),
                _buildInventoryActionCard('Sunflower Oil 1L', '2 Units', '1 Day', OperationalStatus.critical),
                const SizedBox(height: 32),

                // Section 5: Workforce Control Panel
                const Text('WORKFORCE CONTROL', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: AppTheme.grey600)),
                const SizedBox(height: 12),
                _buildWorkforcePanel(),
                const SizedBox(height: 32),

                // Section 6: Actionable Work Queue
                const Text('MY WORK QUEUE', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: AppTheme.grey600)),
                const SizedBox(height: 12),
                UniversalWorkQueueUI(
                  ...
                  onTaskAction: (task) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Action selected for ${task.title}')));
                  },
                ),
              ],
            ),
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              Text('${health.overallScore.toInt()}', style: const TextStyle(fontSize: 48, fontWeight: FontWeight.bold, color: AppTheme.primary)),
  ...
  Widget _buildHealthIndicator(String label, double health) {
    ...
    } else if (health < 90) color = OperationalStatus.warning.color;
        ...
        Stack(
          ...
          children: [
            SizedBox(
              width: 50,
              height: 50,
              child: CircularProgressIndicator(value: health / 100, color: color, backgroundColor: Colors.grey.shade200),
            ),
            Text('${health.toInt()}', style: TextStyle(color: color, fontWeight: FontWeight.bold)),
          ],
        ),
  ...
  Widget _buildInventoryActionCard(String item, String remaining, String stockout, OperationalStatus status) {
    return Card(
      margin: const EdgeInsets.only(bottom: 12),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: BorderSide(color: status.color.withValues(alpha: 0.5))),
      child: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            Text('Predicted Stockout: $stockout', style: TextStyle(color: status.color, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Row(
              children: [
                Expanded(child: ElevatedButton(onPressed: () {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Purchase Request Executed')));
                }, style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primary, foregroundColor: Colors.white), child: const Text('Purchase Request'))),
                const SizedBox(width: 8),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                ...
                onPressed: () {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Contacting Employee')));
                },
