  ...
  Widget _buildMemberCard(FamilyMember member, FamilyMember? currentMember) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: roleColors[member.role]!.withOpacity(0.3),
          width: 1,
        ),
      ),
      child: Row(
        children: [
          CircleAvatar(
            radius: 24,
            backgroundColor: roleColors[member.role]!.withOpacity(0.2),
            child: Text(
    ...
    if (group.sharedCart.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.shopping_cart_outlined, size: 64, color: Colors.white.withOpacity(0.2)),
            const SizedBox(height: 16),
            Text(
