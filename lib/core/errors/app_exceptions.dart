...
class ErrorMapper {
  static AppException map(dynamic error, StackTrace? stackTrace) {
    ...
    if (errStr.contains('network') || errStr.contains('socket') || errStr.contains('timeout')) {
      return NetworkError('Network connectivity issue', originalError: error, stackTrace: stackTrace);
    }
    
    if (errStr.contains('firestore') || errStr.contains('postgres') || errStr.contains('sqlite')) {
      return DatabaseError('Database operation failed', originalError: error, stackTrace: stackTrace);
    }

    // Default fallback
    return AppExceptionWrapper('An unexpected error occurred: ${error.toString()}', originalError: error, stackTrace: stackTrace);
  }
}
