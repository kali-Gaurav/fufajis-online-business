...
CREATE TABLE IF NOT EXISTS product_inventory (
  ...
  -- Three-layer stock model (FIX #4: Added CHECK constraints)
  available_stock INT NOT NULL DEFAULT 0 CHECK (available_stock >= 0),  -- Can be ordered
  reserved_stock INT NOT NULL DEFAULT 0 CHECK (reserved_stock >= 0),   -- Checkout pending payment
  sold_stock INT NOT NULL DEFAULT 0 CHECK (sold_stock >= 0),       -- Payment confirmed
  ...
  -- Ensure reserved + sold <= total stock (prevents data corruption)
  CONSTRAINT valid_stock_allocation CHECK (
    (available_stock + reserved_stock + sold_stock) >= 0
  )
);
