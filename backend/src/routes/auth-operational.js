...
router.post('/operational/request-password-reset', passwordResetLimiter, async (req, res) => {
  try {
    ...
    const resetLink = `${process.env.APP_BASE_URL || 'https://app.fufaji.com'}/auth/reset-password?token=${resetToken}`;
    await sendEmail(
      email,
      process.env.SENDGRID_PASSWORD_RESET_TEMPLATE_ID || 'd-PASSWORD_RESET_TEMPLATE',
      {
        full_name: user.full_name,
